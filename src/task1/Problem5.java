...
public class Problem5 {
	public static void longestDecSubarray(int arr[], int n) {
		...
		int max = 1, len = 1, maxIndex = 0; // max will store that maximum length of subarray, len will store the length
											// of subarray
		...
		for (int i = 1; i < n; i++) { // traverse the array from the 2nd element
			if (arr[i] < arr[i - 1]) { // if current element if greater than previous element then len++
				...
				if (max < len) { // update the max if len is greater than max
					max = len;
					maxIndex = i - max; // assign the index of maximum number in the subarray by calculate the deffrence
										// between current index and max length
				}
	...
	public static void main(String[] args) {
		// Problem 5: Given an array of integers, write a function to find the longest
		// decreasing subarray.
